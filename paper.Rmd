...
author: 
 ...
 - Richard K. Morgan^[VDEM, rick.morgan2@gmail.com.]
 - Michael D. Ward^[Predictive Heuristics, Duke University, University of Washington, michael.don.ward@gmail.com]
date: "`r format(Sys.time(), '%d %B %Y')`"
thanks: Cassy L. Dorff and Shahryar Minhas both provided helpful feedback on this project. All the code and several additional analyses can be found at our replication archive.
...
The most consequential issue that we found is that all AUC-ROC values reported in B&S Tables 1 and 2 are calculated using smoothed ROC curves, not the original, actual ROC curves. The data in rare events problems like this one restricts the number of distinct true positive rate values. As we show below, the use of smoothed ROC curves to calculate AUC has a dramatic impact on the results. To our knowledge, the norm is to calculate AUC-ROC values using original, not smoothed ROC curves. In fact, we are not aware of other work that uses smoothed ROC curves for AUC calculations. It might be that theoretical reasons are justifying the use of smoothed ROC curves over the original ROC curves; but given that this decision dramatically impacts the interpretation of the B&S results, it minimally would have warranted an explicit discussion in the paper. As it is, B&S offer no justification for using smoothed ROC curves in the AUC-ROC calculations.  

Smoothing of ROC curves is well known.  Basically, the smoothed curves transform the false positive rate and the false negative rates jointly in a monotone fashion.  The assumption is that there is a monotone transformation that produces normal distributions for the sensitivity and specificity. It is well-known that there is substantial bias that can be introduced by bias. Indeed, bootstrapping is often used as a diagnostic to see the amount of bias introduced by smoothing.
...
@hegre:etal:2019, 
@hegre:etal:2019.
Indeed this scholarship has brought us to the point where predictive analysis of conflict studies is not unknown in the scholarly and policy arenas.  Most, if not all of these authors, would probably claim some theoretical motivation. The a-theoretical case is a counterfactual that either includes everything, or nothing, as aruged in @ward:2016. In the B&S article it serves as a strawman. Unfortunately for B&S that strawman outperforms their putatively theoretical model.^[Important statistical developments have also added to this research vector, including especially 
