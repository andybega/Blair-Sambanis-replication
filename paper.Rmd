...
The most consequential issue that we found is that all AUC-ROC values reported in B&S Tables 1 and 2 are calculated using smoothed ROC curves, not the original, empirical ROC curves. The data in rare events problems like this one restricts the number of distinct true positive rate values. As we show below, the use of smoothed ROC curves to calculate AUC has a dramatic impact on the results. 

Although smoothing might appear to be a simple option to aid in the visual interpretation of ROC curves, and is presented as such by B&S, it actually involves estimating an underlying population ROC curve, using a parametric binormal model in this case. There is a literature in statistics and the medical field on such ROC estimators and the conditions under which they might be preferable to empirical ROC curves. For example, when the goal is to extrapolate from a statistical sample to a larger population of potential cases... 

(RM: IDK. But I think it can be short and simple. Highlight that smoothing in unconventional in conflict/political forecasting)

The option to smooth the ROC curves is presented by B&S as a trivial choice to aid visual interpretation, but it actually delves into a broader area of parametric or non-parametric population ROC curve estimation. A common motivating scenario is in the area of medical testing, where test results might only be binary or ordinal, not continuous, where only a small number of tests with verified accuracy might be available, and where there is a clear larger population one wants to infer to. The particular application here deviates in several ways from the conditions under which the properties of these estimators have been studies: rare events, our data is more of a census than independent samples from a much larger population, and the use of non-deterministic random forest models. 

Far from being a trivial choice, the question of whether to use smoothed ROC estimates rather than the empirical ROC curves is a major decision that dramatically impacts the results. B&S do not offer a justification for departing from the common practice of using the empirical ROC curves. 
...
tab4f <- tab4f %>%
  rename(`Predicted 0` = `0`, `Predicted 1` = `1`) %>%
...
B&S advocate the use theory to guide prediction. But theory as used by B&S is an ambiguous and undefined concept.  It is not a procedure.  They actually create a model with ten right hand side variables that are supposed to capture a complicated repression-dissent dynamic. There is wide-ranging literature on this dynamic that could justify many specifications.  As such, their baseline comparison is an unfortunate standard bearer for strong theory.
