...
# function to adjust the "cases" labels in Table 2
fix_case_labels <- function(x) {
  # escape the opening and closing parentheses
  stringr::str_replace_all(x, "([\\(\\)])", "\\\\\\1")
}

# function to adjust the model/column labels in Table 2
```


Issues in the B&S replication:

- The AUC-ROC statistics are calculated using smoothed ROC curves
- The "Weighted by PITF" model is incorrectly implemented
- The "PITF Split Population" model is incorrectly implemented
- The forecasts are scored using incidence, not onset

Pre-registration should have had:

- The escalation model specification, prior to looking at data!
- The hyper-parameter tuning strategy. 

With rare events, also a lot of variance over RNG state. 
...
table2_full %>%
  ...
  knitr::kable("latex", booktabs = TRUE, digits = 0,
