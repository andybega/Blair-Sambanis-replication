...
![Copy of B&S Table 1](figures/bands-table1.png)
...
```{r, message=FALSE}
table1 <- read_csv("data/table1-redone.csv") %>%
...
![Copy of B&S Table 2](figures/bands-table2.png)
...
```{r, message=FALSE}
escalation_only <- read_csv("data/table1-redone.csv") %>%
  filter(specification=="escalation") %>%
  select(-model) %>%
  mutate(horizon = case_when(horizon=="6 month" ~ "6 months", TRUE ~ horizon))

table2 <- read_csv("data/table2-redone.csv") %>%
  bind_rows(escalation_only) %>%
  mutate(
    specification = factor(
      specification, 
      levels = c("escalation", "escalation with PITF", 
                 "escalation with PITF weights", 
                 "escalation with Split PITF", "PITF"),
      labels = c("Escalation\nOnly", "With PITF Predictors", "Weighted by PITF", "PITF Split Population", "PITF Only"))
  ) %>%
  arrange(horizon, specification) %>% 
  pivot_longer(-c(horizon, specification), names_to = "ROC Smoothed") %>% 
...
table2 %>%
  knitr::kable(
    ...
    caption = "Comparison of smoothed and not smoothed AUC-ROC values for B\\&S Table 2", 
    align = c(rep("l", 2), rep("c", 6))
  ) %>% 
  kableExtra::column_spec(c(2, 5, 7), width = "1.5cm") %>%
  kableExtra::column_spec(3, width = "1.8cm") %>%
  kableExtra::column_spec(c(4, 6), width = "2.2cm") %>%
