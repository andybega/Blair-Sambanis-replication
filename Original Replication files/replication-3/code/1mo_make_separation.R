    ...
    # Load modified separation plot function from "separationplot" package
      ...
      separationplot_alt <-
        function(pred, actual, type="line", line=T, lwd1=0.5, lwd2=1, heading="", xlab="", shuffle=T, width=8, height=2, col0="#FEF0D9", col1="#E34A33", flag=NULL, flagcol=1, file=NULL, newplot=T, locate=NULL, rectborder=NA, show.expected=F, zerosfirst=T, BW=F, family = "Times"){
          ...
          if (type!="bands"){	 			
            plot(1:nrow(resultsmatrix),1:nrow(resultsmatrix), xlim=c(0.5, nrow(resultsmatrix)+0.5), ylim=c(0,0.02),  type="n", bty="n", yaxt="n", xaxt="n", xlab=xlab, ylab="", family = "Times") #RB: Change ylim to alter y-axis
            title(main=heading, family = "Times")
          } 
          ...
          if (type=="localaverage"){
            ...
            plot(1:length(phat.events),1:length(phat.events), xlim=c(0.5, length(phat.events)+0.5), ylim=c(0,1),  type="n", bty="n", yaxt="n", xaxt="n", xlab=xlab, ylab="", family = "Times")
            ...
            plot(1:length(phat.nonevents),1:length(phat.nonevents), xlim=c(0.5, length(phat.nonevents)+0.5), ylim=c(0,1),  type="n", bty="n", yaxt="n", xaxt="n", xlab=xlab, ylab="", family = "Times")
  ...
  # Drop missing values
    ...
    test_DV_civil_ns_escalation <-test_DV_civil_ns
      test_DV_civil_ns_escalation[is.na(prediction_escalation_inc_civil_ns)] <- NA
      test_DV_civil_ns_escalation <- as.vector(na.omit(test_DV_civil_ns_escalation))

    test_DV_civil_ns_quad <-test_DV_civil_ns
      test_DV_civil_ns_quad[is.na(prediction_quad_inc_civil_ns)] <- NA
      test_DV_civil_ns_quad <- as.vector(na.omit(test_DV_civil_ns_quad))

    test_DV_civil_ns_goldstein <-test_DV_civil_ns
      test_DV_civil_ns_goldstein[is.na(prediction_goldstein_inc_civil_ns)] <- NA
      test_DV_civil_ns_goldstein <- as.vector(na.omit(test_DV_civil_ns_goldstein))

    test_DV_civil_ns_all_CAMEO <-test_DV_civil_ns
      test_DV_civil_ns_all_CAMEO[is.na(prediction_all_CAMEO_inc_civil_ns)] <- NA
      test_DV_civil_ns_all_CAMEO <- as.vector(na.omit(test_DV_civil_ns_all_CAMEO))

    test_DV_civil_ns_avg <-test_DV_civil_ns
      test_DV_civil_ns_avg[is.na(prediction_avg_inc_civil_ns)] <- NA
      test_DV_civil_ns_avg <- as.vector(na.omit(test_DV_civil_ns_avg))

    prediction_escalation_inc_civil_ns <- as.vector(na.omit(prediction_escalation_inc_civil_ns))

    prediction_quad_inc_civil_ns <- as.vector(na.omit(prediction_quad_inc_civil_ns))

    prediction_goldstein_inc_civil_ns <- as.vector(na.omit(prediction_goldstein_inc_civil_ns))

    prediction_all_CAMEO_inc_civil_ns <- as.vector(na.omit(prediction_all_CAMEO_inc_civil_ns))

    prediction_avg_inc_civil_ns <- as.vector(na.omit(prediction_avg_inc_civil_ns))

    
           
  # Generate separation plots
      
      separationplot_alt(pred=prediction_escalation_inc_civil_ns, actual=test_DV_civil_ns_escalation, type="line",
                         line=TRUE, show.expected=FALSE, xlab = "", heading="Escalation", shuffle = F, BW=T, file="figures/figure3_a.pdf")
      
      separationplot_alt(pred=prediction_quad_inc_civil_ns, actual=test_DV_civil_ns_quad, type="line",
                         line=TRUE, show.expected=FALSE, xlab = "", heading="Quad", shuffle = F, BW=T, file="figures/figure3_b.pdf")
      
      separationplot_alt(pred=prediction_goldstein_inc_civil_ns, actual=test_DV_civil_ns_goldstein, type="line",
                         line=TRUE, show.expected=FALSE, xlab = "", heading="Goldstein", shuffle = F, BW=T, file="figures/figure3_c.pdf")
      
      separationplot_alt(pred=prediction_all_CAMEO_inc_civil_ns, actual=test_DV_civil_ns_all_CAMEO, type="line",
                         line=TRUE, show.expected=FALSE, xlab = "", heading="CAMEO", shuffle = F, BW=T, file="figures/figure3_d.pdf")
      
      separationplot_alt(pred=prediction_avg_inc_civil_ns, actual=test_DV_civil_ns_avg, type="line",
                         line=TRUE, show.expected=FALSE, xlab = "", heading="Average", shuffle = F, BW=T, file="figures/figure3_e.pdf")
