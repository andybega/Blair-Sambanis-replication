...
RF_ranking = matrix(NA,nrow=length(escalation_train_RHS), ncol=1)
...
rownames(RF_ranking) <- escalation
...
#   			prediction_robust_maxnodes <- as.numeric(predict(train_model_robust_maxnodes, newdata = data.matrix(escalation_test_frame_civil_ns), type="response"))
# 				  prediction_robust_maxnodes[is.na(escalation_test_frame_civil_ns$test_DV_civil_ns)] <- NA
#
#       	prediction_robust_sampsize <- as.numeric(predict(train_model_robust_sampsize, newdata = data.matrix(escalation_test_frame_civil_ns), type="response"))
# 				  prediction_robust_sampsize[is.na(escalation_test_frame_civil_ns$test_DV_civil_ns)] <- NA
#
#       	prediction_robust_ntree <- as.numeric(predict(train_model_robust_ntree, newdata = data.matrix(escalation_test_frame_civil_ns), type="response"))
# 				  prediction_robust_ntree[is.na(escalation_test_frame_civil_ns$test_DV_civil_ns)] <- NA
#
#         prediction_robust_traintest1 <- as.numeric(predict(train_model_robust_traintest1, newdata = data.matrix(escalation_test_frame_civil_ns_robust_traintest1), type="response"))
# 				  prediction_robust_traintest1[is.na(escalation_test_frame_civil_ns_robust_traintest1$test_DV_civil_ns_robust_traintest1)] <- NA
#
# 				prediction_robust_traintest2 <- as.numeric(predict(train_model_robust_traintest2, newdata = data.matrix(escalation_test_frame_civil_ns_robust_traintest2), type="response"))
# 				  prediction_robust_traintest2[is.na(escalation_test_frame_civil_ns_robust_traintest2$test_DV_civil_ns_robust_traintest2)] <- NA
#
# 				prediction_robust_traintest3 <- as.numeric(predict(train_model_robust_traintest3, newdata = data.matrix(escalation_test_frame_civil_ns_robust_traintest3), type="response"))
# 				  prediction_robust_traintest3[is.na(escalation_test_frame_civil_ns_robust_traintest3$test_DV_civil_ns_robust_traintest3)] <- NA
#
# 				prediction_robust_civil_ns_alt1 <- as.numeric(predict(train_model_robust_civil_ns_alt1, newdata = data.matrix(escalation_test_frame_civil_ns_alt1), type="response"))
# 				  prediction_robust_civil_ns_alt1[is.na(escalation_test_frame_civil_ns_alt1$test_DV_civil_ns_alt1)] <- NA
#
# 				prediction_robust_civil_ns_alt2 <- as.numeric(predict(train_model_robust_civil_ns_alt2, newdata = data.matrix(escalation_test_frame_civil_ns_alt2), type="response"))
# 				  prediction_robust_civil_ns_alt2[is.na(escalation_test_frame_civil_ns_alt2$test_DV_civil_ns_alt2)] <- NA

# Calculate AUC

performance <- prediction(prediction, test_DV_civil_ns)
roc <- roc(test_DV_civil_ns, prediction, smooth=TRUE, auc = TRUE)
saveRDS(roc, "figures/roc-1mo-escalation.rds")
AUC_obs <- data.frame(as.numeric(roc$auc))

## RM: ADDED 4/29/2020
roc_noSmooth <- roc(test_DV_civil_ns, prediction, smooth=FALSE, auc = TRUE)
saveRDS(roc_noSmooth, "figures/non-smooth-roc-1mo-escalation.rds")
...
pred_escalation_inc_civil_ns <- data.frame(cbind(country, year, month, period,
                                                 incidence_civil_ns, incidence_civil_ns_plus1,
                                                 prediction))

colnames(pred_escalation_inc_civil_ns) <- c("country", "year", "month", "period",
                                            "incidence_civil_ns", "incidence_civil_ns_plus1",
                                            "prediction")
...
# prediction_robust_traintest1_escalation_inc_civil_ns <- prediction_robust_traintest1
# prediction_robust_traintest2_escalation_inc_civil_ns <- prediction_robust_traintest2
# prediction_robust_traintest3_escalation_inc_civil_ns <- prediction_robust_traintest3
# prediction_robust_civil_ns_alt1_escalation_inc_civil_ns <- prediction_robust_civil_ns_alt1
# prediction_robust_civil_ns_alt2_escalation_inc_civil_ns <- prediction_robust_civil_ns_alt2
roc_escalation_inc_civil_ns <- roc
roc_noSmooth_escalation_inc_civil_ns <- roc_noSmooth
# precision_recall_escalation_inc_civil_ns <- precision_recall
# brier_obs_escalation_inc_civil_ns <- brier_obs
# RF_ranking_escalation_inc_civil_ns <- RF_ranking
# f1_max_escalation_inc_civil_ns <- f1_max
