...
#   			train_model_robust_maxnodes <- randomForest(quad_train_formula_civil_ns,
# 												  type = regression, importance = TRUE,
...
# 												  )
#
#     		train_model_robust_sampsize <- randomForest(quad_train_formula_civil_ns,
# 												  type = regression, importance = TRUE,
# 												  na.action = na.omit, ntree = 100000, maxnodes = 5,
# 												  sampsize = 500,
# 												  replace = FALSE, do.trace = FALSE ,
...
# 												  )
#
#   			train_model_robust_ntree <- randomForest(quad_train_formula_civil_ns,
# 												  type = regression, importance = TRUE,
...
# 												  )
#
#     		train_model_robust_traintest1 <- randomForest(quad_train_formula_civil_ns_robust_traintest1,
# 												  type = regression, importance = TRUE,
# 												  na.action = na.omit, ntree = 100000, maxnodes = 5,
# 												  sampsize = 100,
# 												  replace = FALSE, do.trace = FALSE ,
# 												  data = quad_train_frame_civil_ns_robust_traintest1, forest = TRUE
# 												  )
#
#     		train_model_robust_traintest2 <- randomForest(quad_train_formula_civil_ns_robust_traintest2,
#     		                  type = regression, importance = TRUE,
#     		                  na.action = na.omit, ntree = 100000, maxnodes = 5,
#     		                  sampsize = 100,
#     		                  replace = FALSE, do.trace = FALSE ,
#     		                  data = quad_train_frame_civil_ns_robust_traintest2, forest = TRUE
#     		                  )
#
#     		train_model_robust_traintest3 <- randomForest(quad_train_formula_civil_ns_robust_traintest3,
#     		                  type = regression, importance = TRUE,
#     		                  na.action = na.omit, ntree = 100000, maxnodes = 5,
#     		                  sampsize = 100,
#     		                  replace = FALSE, do.trace = FALSE ,
#     		                  data = quad_train_frame_civil_ns_robust_traintest3, forest = TRUE
#     		                  )
#
#       	train_model_robust_civil_ns_alt1 <- randomForest(quad_train_formula_civil_ns_alt1,
#       	                  type = regression, importance = TRUE,
#       	                  na.action = na.omit, ntree = 100000, maxnodes = 5,
#       	                  sampsize = 100,
#       	                  replace = FALSE, do.trace = FALSE ,
#       	                  data = quad_train_frame_civil_ns_alt1, forest = TRUE
#       	                  )
#
#       	train_model_robust_civil_ns_alt2 <- randomForest(quad_train_formula_civil_ns_alt2,
#       	                  type = regression, importance = TRUE,
#       	                  na.action = na.omit, ntree = 100000, maxnodes = 5,
#       	                  sampsize = 100,
#       	                  replace = FALSE, do.trace = FALSE ,
#       	                  data = quad_train_frame_civil_ns_alt2, forest = TRUE
#       	                  )
...
RF_ranking = matrix(NA,nrow=length(quad_train_RHS), ncol=1)
...
rownames(RF_ranking) <- quad
RF_ranking <- data.frame(RF_ranking)


# Generate out-of-sample predictions

prediction <- as.numeric(predict(train_model, newdata = data.matrix(quad_test_frame_civil_ns), type="response"))
prediction[is.na(quad_test_frame_civil_ns$test_DV_civil_ns)] <- NA

#   			prediction_robust_maxnodes <- as.numeric(predict(train_model_robust_maxnodes, newdata = data.matrix(quad_test_frame_civil_ns), type="response"))
# 				  prediction_robust_maxnodes[is.na(quad_test_frame_civil_ns$test_DV_civil_ns)] <- NA
#
#       	prediction_robust_sampsize <- as.numeric(predict(train_model_robust_sampsize, newdata = data.matrix(quad_test_frame_civil_ns), type="response"))
# 				  prediction_robust_sampsize[is.na(quad_test_frame_civil_ns$test_DV_civil_ns)] <- NA
#
#       	prediction_robust_ntree <- as.numeric(predict(train_model_robust_ntree, newdata = data.matrix(quad_test_frame_civil_ns), type="response"))
# 				  prediction_robust_ntree[is.na(quad_test_frame_civil_ns$test_DV_civil_ns)] <- NA
#
#         prediction_robust_traintest1 <- as.numeric(predict(train_model_robust_traintest1, newdata = data.matrix(quad_test_frame_civil_ns_robust_traintest1), type="response"))
# 				  prediction_robust_traintest1[is.na(quad_test_frame_civil_ns_robust_traintest1$test_DV_civil_ns_robust_traintest1)] <- NA
#
# 				prediction_robust_traintest2 <- as.numeric(predict(train_model_robust_traintest2, newdata = data.matrix(quad_test_frame_civil_ns_robust_traintest2), type="response"))
# 				  prediction_robust_traintest2[is.na(quad_test_frame_civil_ns_robust_traintest2$test_DV_civil_ns_robust_traintest2)] <- NA
#
# 				prediction_robust_traintest3 <- as.numeric(predict(train_model_robust_traintest3, newdata = data.matrix(quad_test_frame_civil_ns_robust_traintest3), type="response"))
# 				  prediction_robust_traintest3[is.na(quad_test_frame_civil_ns_robust_traintest3$test_DV_civil_ns_robust_traintest3)] <- NA
#
# 				prediction_robust_civil_ns_alt1 <- as.numeric(predict(train_model_robust_civil_ns_alt1, newdata = data.matrix(quad_test_frame_civil_ns_alt1), type="response"))
# 				  prediction_robust_civil_ns_alt1[is.na(quad_test_frame_civil_ns_alt1$test_DV_civil_ns_alt1)] <- NA
#
# 				prediction_robust_civil_ns_alt2 <- as.numeric(predict(train_model_robust_civil_ns_alt2, newdata = data.matrix(quad_test_frame_civil_ns_alt2), type="response"))
# 				  prediction_robust_civil_ns_alt2[is.na(quad_test_frame_civil_ns_alt2$test_DV_civil_ns_alt2)] <- NA
...
saveRDS(roc_noSmooth, "figures/non-smooth-roc-1mo-quad.rds")
...
write.csv(tbl, "tables/auc-1mo-quad.csv", row.names = FALSE)
...
pred_quad_inc_civil_ns <- data.frame(cbind(country, year, month, period,
                                           incidence_civil_ns, incidence_civil_ns_plus1,
                                           prediction))

colnames(pred_quad_inc_civil_ns) <- c("country", "year", "month", "period",
                                      "incidence_civil_ns", "incidence_civil_ns_plus1",
                                      "prediction")
...
AUCs_quad <- AUCs
prediction_quad_inc_civil_ns <- prediction
# prediction_robust_maxnodes_quad_inc_civil_ns <- prediction_robust_maxnodes
# prediction_robust_sampsize_quad_inc_civil_ns <- prediction_robust_sampsize
# prediction_robust_ntree_quad_inc_civil_ns <- prediction_robust_ntree
# prediction_robust_traintest1_quad_inc_civil_ns <- prediction_robust_traintest1
# prediction_robust_traintest2_quad_inc_civil_ns <- prediction_robust_traintest2
# prediction_robust_traintest3_quad_inc_civil_ns <- prediction_robust_traintest3
# prediction_robust_civil_ns_alt1_quad_inc_civil_ns <- prediction_robust_civil_ns_alt1
# prediction_robust_civil_ns_alt2_quad_inc_civil_ns <- prediction_robust_civil_ns_alt2
roc_quad_inc_civil_ns <- roc
roc_noSmooth_quad_inc_civil_ns <- roc_noSmooth
# precision_recall_quad_inc_civil_ns <- precision_recall
# brier_obs_quad_inc_civil_ns <- brier_obs
# RF_ranking_quad_inc_civil_ns <- RF_ranking
# f1_max_quad_inc_civil_ns <- f1_max
#
