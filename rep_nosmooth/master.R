...
AUCs <- as.matrix(cbind(AUCs_escalation, AUCs_quad, AUCs_goldstein, AUCs_all_CAMEO, AUCs_avg))
colnames(AUCs) <- c("escalation", "quad", "Goldstein", "CAMEO", "avg")
rownames(AUCs) <- c("Smoothed","Not Smoothed")

write.csv(AUCs, file = "tables/table1_top.csv", row.names = T)

# Create left panel of Figure 1

##I'm going to remake figure one in a different script, one that includes panels for 1 and 6 mon target variables for both smoothed and non-smoothed ROCs. Rather than joining the panels in post production.
...
## RM: ADDED 4/29/2020
AUCs_6mo <- as.matrix(cbind(AUCs_escalation, AUCs_quad, AUCs_goldstein, AUCs_all_CAMEO, AUCs_avg))
colnames(AUCs_6mo) <- c("escalation", "quad", "Goldstein", "CAMEO", "avg")
rownames(AUCs_6mo) <- c("Smoothed","Not Smoothed")
