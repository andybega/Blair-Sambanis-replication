...
train_model <- randomForest(PITF_6mo_train_formula_civil_ns,
                            type = regression, importance = TRUE,
                            na.action = na.omit, ntree = 100000, maxnodes = 5,
                            sampsize = 100,
                            replace = FALSE, do.trace = FALSE ,
                            data = PITF_6mo_train_frame_civil_ns, forest = TRUE
)


# Generate out-of-sample predictions

prediction <- as.numeric(predict(train_model, newdata = data.matrix(PITF_6mo_test_frame_civil_ns), type="response"))
prediction[is.na(PITF_6mo_test_frame_civil_ns$test_DV_civil_ns)] <- NA
